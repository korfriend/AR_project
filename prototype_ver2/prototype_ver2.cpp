		...
		if (trk_info.is_updated && current_frameset && current_depth_frame)
		{
			...
			*/
			...
			if (trk_info.is_detected_sshead)
			{
				...
				static int ssu_tool_line_ms_id = 0, ssu_tool_sphere_zs_id = 0;
				if (trk_info.is_detected_sstool && ss_tool_info.pos_centers_tfrm.size()) {
					...
					// zs
					vzm::GenerateSpheresObject(__FP glm::fvec4(sstool_p1_ws, 0.0015f), __FP glm::fvec3(0, 1, 1), 1, ssu_tool_sphere_zs_id);
					vzm::ObjStates zoom_state = obj_state;
					zoom_state.color[3] = 0.4;
					vzm::ReplaceOrAddSceneObject(g_info.zoom_scene_id, ssu_tool_sphere_zs_id, zoom_state);
					...
					if (bAlign) {
						...
						cyl_r = 1.5f;
						vzm::ObjStates ssu_tool_line_ms_state = model_state;

						vzm::GenerateCylindersObject((float*)cyl_p03, &cyl_r, __FP cyl_rgb, 1, ssu_tool_line_ms_id);
						vzm::ReplaceOrAddSceneObject(g_info.model_scene_id, ssu_tool_line_ms_id, ssu_tool_line_ms_state);
					}
				}

				/////////////////////////////////////////////////////////////////////////////////
				// draw ssu_tool guide dir
				static int section_ssu_tool_guide_id = 0, section_ssu_tool_guide_ws_id = 0, section_ssu_tool_guide_zs_id = 0;
				if (trk_info.is_detected_sstool && bAlign) {
					...
					if (tool_guide_pos_os.size() > 0) {
						...
						vzm::GenerateCylindersObject((float*)cyl_p02, &cyl_r, __FP cyl_rgb2, 1, section_ssu_tool_guide_zs_id);
						vzm::ReplaceOrAddSceneObject(g_info.zoom_scene_id, section_ssu_tool_guide_zs_id, ssu_tool_guide_zs_state);
					...
					if (tool_guide_pos_os.size() && ss_tool_info.pos_centers_tfrm.size()) {
						...
						if (show_guide_view) {
							...
							*/
							...
							// draw angle(line + arc) ///////////////////////////////////////////////////////////////
							vzm::ObjStates angleLineState = obj_state;
							...
							//clr_lines.emplace_back(glm::fvec3(0, 0, 1));

							__cm4__ angleLineState.os2ws = glm::fmat4(1.f);
							angleLineState.line_thickness = 5;
							...
							*/
							...
							auto make_angluar_tris = [](std::vector<glm::fvec3>& pos_tris, const glm::fvec3 pos_ctrs[3]) {
								using namespace glm;
								fvec3 vec01 = pos_ctrs[1] - pos_ctrs[0];
								fvec3 vec02 = pos_ctrs[2] - pos_ctrs[0];
								float btw_angle = acos(dot(vec01, vec02) / (length(vec01) * length(vec02)));
								const float inter_angle = glm::pi<float>() / 72.f; // 5 deg
								int num_intervals = (int)(btw_angle / inter_angle);
								fvec3 rot_axis = normalize(cross(vec01, vec02));

								pos_tris.clear();
								pos_tris.push_back(pos_ctrs[0]);
								pos_tris.push_back(pos_ctrs[1]);
								for (int i = 0; i < num_intervals; i++) {
									fmat3x3 rot_mat = glm::rotate(i*inter_angle, rot_axis);
									fvec3 pos_r = rot_mat * vec01 + pos_ctrs[0];
									pos_tris.push_back(pos_r);
							...
							for (int i = 0; i < pos_tris.size(); ++i)
								clr_tris.emplace_back(glm::fvec3(0.98, 0.75, 0.16));

							__cm4__ angleArcState.os2ws = glm::fmat4(1.f);
							angleArcState.color[3] = 0.5f;
							...
							*/
							...
							vzm::ObjStates angleArrowState = obj_state;
							...
							auto make_angle_guide = [&guide_dir_ws, &tool_dir_ws](glm::fvec3& pos_s, glm::fvec3& pos_e, glm::fvec4& color, const glm::fvec3& up, const glm::fvec3& right, const glm::fvec3& pos) {
								...
								if (y > 0)
									color = glm::fvec4(0, 1, 0, 0.8);
								else
									color = glm::fvec4(0, 0, 1, 0.8);
							};
